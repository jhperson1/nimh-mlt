...
def visREG(NOISE, adv, LAMBDA, epsilon, REG, SEED):
    ...
    fn = SUFFIX+"_SEED"+str(SEED)+"_layers"+str(LAYERS)+"_"+REG+"_lambda"+str(LAMBDA)+"_epsilon"+str(epsilon)+"_epsilonSG"+str(epsilonSG)+"_noise"+NOISE+"_sign"+adv+"_units"+str(UNITS)+"_grads"
    ...
    grads = np.load(fn+".npy")
    # hesss = np.empty(shape=(10,5,5))

    means = grads.mean(axis=0)
    stds = grads.std(axis=0)

    poss = np.zeros(shape = means.shape)
    negs = np.zeros(shape = means.shape)

    for i in range(nFEATS):
        poss[i] = posrate(grads[i])
        negs[i] = negrate(grads[i])


    with open('gradient_results.csv', 'w') as f:
        fw = csv.writer(f)
        fw.writerow(['means'])
        fw.writerow(fnames)
        fw.writerows(means)
            ...
            for LAMBDA in LAMBDAs:
                for epsilon in epsilons:
                    for epsilonSG in epsilonSGs:
                        # positive ratio
                        pregs = np.zeros(shape=(nSEEDS, nFEATS, nFEATS))
                        # negative ratio
                        nregs = np.zeros(shape=(nSEEDS, nFEATS, nFEATS))
                        # positive ratio minus negative ratio
                        pmnregs = np.zeros(shape=(nSEEDS, nFEATS, nFEATS))
                        # means
                        mregs = np.zeros(shape=(nSEEDS, nFEATS, nFEATS))
                        for SEED in range(1,nSEEDS+1):
                            print NOISE, adv, LAMBDA, epsilon, SEED
                            mreg, sreg, preg, nreg = visREG(NOISE, adv, LAMBDA, epsilon, REG, SEED)
                            pregs[SEED - 1, :, :] = preg
                            nregs[SEED - 1, :, :] = nreg
                            pmnregs[SEED - 1, :, :] = preg - nreg
                            mregs[SEED - 1, :, :] = mreg
                        ...
                        cmap = sns.diverging_palette(250, 10, as_cmap=True)
                        ax = sns.heatmap(mregs.mean(axis=0), center=0, annot=True, fmt=".4f",
                                         yticklabels=fnames , cmap=cmap) # xticklabels=fnames,
                        ax.set_title("gradient average gradient value across " +str(nSEEDS)+ " resamplings")
                        plt.show()

                        cmap = sns.diverging_palette(250, 10, as_cmap=True)
                        ax = sns.heatmap(pmnregs.mean(axis=0), center=0, annot=True, fmt=".4f",
                                         yticklabels=fnames , cmap=cmap) # xticklabels=fnames,
                        ax.set_title("gradient expected prevalence across " +str(nSEEDS)+ " resamplings")
                        plt.show()

                        cmap = sns.diverging_palette(250, 10, as_cmap=True)
                        ax = sns.heatmap(pregs.mean(axis=0), center = 0.5, annot=True, fmt=".3f",
                                         yticklabels=fnames , cmap=cmap) # xticklabels=fnames,
                        ax.set_title("gradient expected positive ratio across " +str(nSEEDS)+ " resamplings")
                        plt.show()

                        cmap = sns.diverging_palette(250, 10, as_cmap=True)
                        ax = sns.heatmap(nregs.mean(axis=0), center = 0.5, annot=True, fmt=".3f",
                                         yticklabels=fnames , cmap=cmap) # xticklabels=fnames,
                        ax.set_title("gradient expected negative ratio across " +str(nSEEDS)+ " resamplings")
                        plt.show()
